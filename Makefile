...
download:
	@./scripts/nexus.py --kubernetes $(KUBE_VERSION) \
		...
		--crio $(CRIO_VERSION) download --quiet
...
upload:
	@./scripts/nexus.py --kubernetes $(KUBE_VERSION) \
		...
		--runc $(RUNC_VERSION) \
		--crictl $(CRICTL_VERSION) \
		--crio $(CRIO_VERSION) upload --url $(NEXUS_DOMAIN_NAME) --repository $(NEXUS_REPOSITORY)